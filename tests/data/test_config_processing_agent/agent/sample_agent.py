...
IMPORTANT: The config writing functions now require a 'force' parameter:
- write_yaml_file: requires force=True to overwrite existing files
- write_toml_file: requires force=True to overwrite existing files
- write_ini_file: requires force=True to overwrite existing files

Always use force=True when writing config files to avoid permission errors.
