...
@strands_tool
def monitor_function_performance(duration_seconds: int = 60) -> Dict[str, Union[int, float, List]]:
    """
    ...
    """
    if not HAS_PSUTIL:
        raise BasicAgentToolsError("psutil package required for performance monitoring - install with: pip install psutil")

    if not isinstance(duration_seconds, int) or duration_seconds < 1 or duration_seconds > 3600:
        raise BasicAgentToolsError("Duration must be an integer between 1 and 3600 seconds")
    ...
    try:
        ...
        sample_interval = min(2, duration_seconds // 10 if duration_seconds >= 10 else 1)
        samples_taken = 0
...
def get_system_load_average() -> Dict[str, Union[float, str, bool]]:
    ...
    """
    if not HAS_PSUTIL:
        raise BasicAgentToolsError("psutil package required for load average - install with: pip install psutil")
    ...
    try:
        ...
        result = {
            ...
            "platform_support": "unix_like" if load_available else "windows_or_unsupported"
        }
...
@strands_tool
def profile_code_execution(code_snippet: str, iterations: int = 1) -> Dict[str, Union[float, str, int, bool]]:
    """
    ...
    dangerous_keywords = ['import os', 'import sys', 'exec(', 'eval(', '__import__', 'open(', 'file(']
    code_lower = code_snippet.lower()
    for keyword in dangerous_keywords:
        if keyword in code_lower:
            raise BasicAgentToolsError(f"Code contains potentially dangerous keyword: {keyword}")
    ...
    try:
        ...
        return {
            ...
            "executions_per_second": round(1 / avg_time, 2) if avg_time > 0 else float('inf'),
...
@strands_tool
def benchmark_disk_io(file_path: str, data_size_kb: int = 1024) -> Dict[str, Union[str, int, float]]:
    """
    ...
    if not isinstance(data_size_kb, int) or data_size_kb < 1 or data_size_kb > 10240:
        raise BasicAgentToolsError("Data size must be an integer between 1 and 10240 KB")
    ...
    try:
        ...
        write_speed_mbps = (data_size_kb / 1024) / write_time if write_time > 0 else float('inf')
        ...
        read_speed_mbps = (data_size_kb / 1024) / read_time if read_time > 0 else float('inf')
